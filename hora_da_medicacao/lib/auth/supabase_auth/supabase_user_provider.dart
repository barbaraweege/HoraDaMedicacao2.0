...
class HoraDaMedicacaoSupabaseUser extends BaseAuthUser {
  HoraDaMedicacaoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> horaDaMedicacaoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HoraDaMedicacaoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
